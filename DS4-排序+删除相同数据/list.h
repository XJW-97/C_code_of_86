...
void SListSort(SList* plist);//进行排序
void SListRemoveAll(SList* plist,ElemType x);//删除所有相同的数据
...
{
	assert(plist);
	if (plist->head == NULL || plist->head->next == NULL)//链表为空，或者链表为单节点时，不进行操作
	{
		return;
	}
	else
	{
		...
		while(p != NULL)
		{
			...
			SListNode* pre = NULL;
			while (s!=NULL && q->data>s->data)//进行插入
			{
				pre = s;
				s = s->next;
			}
			if (pre == NULL)//说明需要进行头插,头发生变化，需要改变头
			{
				q->next = s;
				plist->head = q;
			}
...
void SListRemoveAll(SList* plist, ElemType x)
{
	...
	else
	{
		...
		{	
			...
			if (pre == NULL)//意味着第一个就是我们要查找的数据
			{
				plist->head = p->next;
			}
			else//查找的数据在之后
			{
				if (p == NULL)//p==NULL的时候，代表没找到
				{
					return;
				}
				else//代表找到了
